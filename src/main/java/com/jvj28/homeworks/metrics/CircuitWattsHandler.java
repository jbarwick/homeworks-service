...
public class CircuitWattsHandler implements MetricHandler {
    ...
    public Set<Metric> getMetricValues(Model model) {
        ...
        Set<Metric> metric_set = new java.util.HashSet<Metric>();
        ...
        model.getCircuits().forEach(c -> {
            metric_set.add(new Metric(Metrics.circuit_watts, getAttributes(c), (int) (c.getLevel() * c.getWatts() / 100.0)));
        });
        return metric_set;
    }

    private LabelList getAttributes(Circuit circuit) {
